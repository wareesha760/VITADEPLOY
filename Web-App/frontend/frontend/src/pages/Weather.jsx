...
const generateHourly = () => {
  const icons = [<FaCloudShowersHeavy color="white" />, <WiCloudy color="white" />];
...
export default function Weather() {
  return (
    <div className="min-h-screen w-screen bg-gradient-to-b from-green-900 to-green-700 text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-1"> کسان بوٹ</h1>
        ...
        <div className="text-center my-6">
          ...
          <div className="text-sm text-gray-200">{weatherData.range}</div>
        </div>
        ...
        <div className="mt-10 w-full max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {weatherData.forecast.map((item, i) => (
              <div
                key={i}
                className="rounded-xl bg-white/10 p-6 shadow-md text-center flex flex-col justify-between min-h-[220px]"
              >
                ...
                <div className="text-xs text-gray-300">{item.note}</div>
              </div>
            ))}
          </div>
        </div>
